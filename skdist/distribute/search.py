...
from .base import _check_estimator, _safe_split
...
def _fit_and_score(estimator, X, y, scorer, train, test, verbose,
    ...
    """
    estimator_ = clone(estimator.value)
    if verbose > 1:
        ...
        if self.refit:
            ...
            if self.preds:
                ...
                for train_index, test_index in cv_splitted:
                    estimator_ = clone(base_estimator).set_params(
                        **self.best_params_)
...
class DistGridSearchCV(DistBaseSearchCV, GridSearchCV):
    ...
    Args:
        ...
        partitions (int or 'auto'): default 'auto'
            ...
            search space. Integer values or None will be used directly for `numSlices`,
            while 'auto' will set `numSlices` to the number required fits.
        **kwargs: Keyword arguments to be passed to `GridSearchCV`.
    """
    def __init__(self, estimator, param_grid, sc=None, partitions='auto', preds=False, **kwargs):
...
class DistRandomizedSearchCV(DistBaseSearchCV, RandomizedSearchCV):
    ...
    Args:
        ...
        partitions (int or 'auto'): default 'auto'
            ...
            search space. Integer values or None will be used directly for `numSlices`,
            while 'auto' will set `numSlices` to the number required fits.
        **kwargs: Keyword arguments to be passed to `RandomizedSearchCV`.
    """
    def __init__(self, estimator, param_distributions, sc=None, partitions='auto', preds=False, **kwargs):
        RandomizedSearchCV.__init__(
            self, estimator, param_distributions, **kwargs)
        self.sc = sc
        self.partitions = partitions
        self.preds = preds
