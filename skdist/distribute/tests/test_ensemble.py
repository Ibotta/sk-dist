...
def test_rfc():
    ...
    y = np.array([0, 1, 0])
    rfc = ensemble.DistRandomForestClassifier(n_estimators=10, random_state=5)
    rfc.fit(X, y)
    preds = rfc.predict(X)
...
def test_rfr():
    ...
    rfr = ensemble.DistRandomForestRegressor(n_estimators=10, random_state=5)
    rfr.fit(X, y_reg)
    preds = rfr.predict(X)
...
def test_etc():
    X = np.array([[0, 1, 0, 1], [0, 0, 0, 1], [1, 0, 1, 0]])
    y = np.array([0, 1, 0])
    etc = ensemble.DistExtraTreesClassifier(n_estimators=10, random_state=5)
    etc.fit(X, y)
    preds = etc.predict(X)
    assert np.allclose(preds, np.array([0, 1, 0]))
...
def test_etr():
    X = np.array([[0, 1, 0, 1], [0, 0, 0, 1], [1, 0, 1, 0]])
    y_reg = np.array([0.1, 0.2, 0.1])
    etr = ensemble.DistExtraTreesRegressor(n_estimators=10, random_state=5)
    etr.fit(X, y_reg)
    preds = etr.predict(X)
...
def test_rte():
    X = np.array([[0, 1, 0, 1], [0, 0, 0, 1], [1, 0, 1, 0]])
    rte = ensemble.DistRandomTreesEmbedding(n_estimators=10, random_state=5)
    rte.fit(X, y=None)
